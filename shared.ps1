...
function Invoke-Strict (
	...
	if(-not $args -or $args.Length -eq 0){
		throw "No command has been provided";
	}
	$cmd = $args[0];
	...
	$text = $args -join ' ';
	# Write-Output "invoking: $text";
	& $cmd @params;
	
	if ($LASTEXITCODE -ne 0) {
		throw "Command ""$text"" failed with exit code $LASTEXITCODE"
	}
}
...
function Confirm-NonProdDotNetEnvironment{
	if ($env:ASPNETCORE_ENVIRONMENT -eq "production" `
		...
		throw "!!!FULL STOP!!! Current ASPNETCORE_ENVIRONMENT is PRODUCTION";
	}

	if ($env:DOTNET_ENVIRONMENT -eq "production" `
			-or $env:DOTNET_ENVIRONMENT -eq "prod")	{
		throw "!!!FULL STOP!!! Current DOTNET_ENVIRONMENT is PRODUCTION";
	}
}